...
app = Flask(__name__, 
            ...
            static_folder=str(basedir / 'static') # Garante que o CSS seja encontrado
            )
...
def pedidos_prontos():
    ...
    for pid, c in DB['carrinhos'].items():
        if c.status == 'PROCESSADO' and c.cliente.cpf == cliente.cpf: # Correção para comparar CPF ou ID
...
def admin_editar_produto(pid):
    ...
    # Se for 'novo', produto será None, o que é esperado para criação
    if pid != 'novo' and not produto:
        return "Produto não encontrado.", 404

    if request.method == 'POST':
        # Se for novo, cria uma instância vazia (ou usa dados do form para criar)
        if pid == 'novo':
             # Gera um novo ID simples
             p_ids = [int(k) for k in DB['produtos'].keys()]
             new_id = str(max(p_ids) + 1) if p_ids else "101"
             
             # Cria o objeto (os dados virão do form logo abaixo)
             # Criamos um placeholder para preencher
             produto = ProdutoFisico("", 0.0, 0.0, "")
             DB['produtos'][new_id] = produto
        
        # Atualiza os dados (Create ou Update)
...
@app.route('/admin/deletar/produto/<pid>', methods=['POST'])
def admin_deletar_produto(pid):
    """(D)ELETE: Rota para excluir um produto."""
    if not session.get('is_admin'):
        return redirect(url_for('index'))
    ...
    if pid in DB['produtos']:
        del DB['produtos'][pid]
        database.salvar_dados_json(DB)
        print(f"[ADMIN] Produto ID {pid} excluído.")
    
    return redirect(url_for('admin_dashboard'))


# --- ROTAS DE DETALHES DO PRODUTO, AVALIAÇÃO E BUSCA ---
...
def detalhe_produto(pid):
    ...
    # Prepara as avaliações para exibição
    ...
    return render_template(
        ...
        avaliacoes=avaliacoes_enriquecidas
    )

@app.route('/produto/<pid>/avaliar', methods=['POST']) 
def avaliar_produto(pid):
    """Rota para processar uma nova avaliação."""
    cliente = get_current_user()
    if not cliente:
